...
public:
	...
	Magnitude() :
		_value(0.0), _unit(0), _empty(true), _correct(false), _corrupt(true) {}

	/*! Crea una magnitud medida en la unidad \a unit, pero sin valor */
	Magnitude(const Unit *unit) :
		_value(0.0), _unit(unit), _empty(true), _correct(unit != 0), _corrupt(unit == 0) {}

	/*! Crea una magnitud con valor \a value y unidad de medida \a unit */
	Magnitude(double value, const Unit *unit) :
		_value(value), _unit(unit), _empty(unit == 0), _correct(unit != 0), _corrupt(unit == 0) {}
	...
	void clean() { _empty = true; _corrupt = (_unit == 0); _correct = !_corrupt; }
	
	void allClean() { _value = 0.0; _unit = 0; _empty = true; _correct = false; _corrupt = true; }

	/*! Fija la unidad de la magnitud */
	void setUnit(const Unit *unit) { _unit = unit; _empty = true; _corrupt = (_unit == 0); _correct = !_corrupt; }
	...
	void setMagnitude(double value, const Unit *unit) {
		_value = value; _unit = unit; _empty = (unit == 0); _correct = (unit != 0); _corrupt = !_correct; }
	...
	static QVariant::Type	typeId() { return _typeId; }

...
class Dato (PySeqTableModel):
	def __init__(self, parent, db = QtSql.QSqlDatabase()):
		...
		self.setSort(self.fieldIndex('idclasificacion'), Qt.SortOrder.AscendingOrder)
		self.app=parent

	def canErase(self, id):
		sql = "select idclasificacion from dato where id = %d" % (id)
		idclas = self.first(sql).value(0)
		if idclas is None or int(idclas) != self.idClasificacion:
			return False
